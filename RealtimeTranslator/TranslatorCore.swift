        ...
        do {
            ...
            for try await token in cloudTranslator.stream(text: utterance.text, from: lang, apiKey: anthropicKey) {
                ...
                while let sentence = Self.takeSentence(&chunk) {
                    state = .playing
                    try await cloudTTS.stream(text: sentence, apiKey: elevenKey, voiceId: elevenVoiceId) { [audio] buffer in
            ...
            if !rest.isEmpty {
                state = .playing
                try await cloudTTS.stream(text: rest, apiKey: elevenKey, voiceId: elevenVoiceId) { [audio] buffer in
                    audio.scheduleStream(buffer, onLeft: playOnLeft)
                }
            }
